...
function frame:OnEvent(event, arg1, arg2)
    if event == "ADDON_LOADED" and arg1 == "Remindr" then
        ...
        if RemindrDB == nil then
            RemindrDB = { version = version, reminders = {} }
        end
    ...
    if event == "PLAYER_ENTERING_WORLD" and arg1 then
        ...
        for name, obj in pairs(RemindrDB.reminders) do
            obj.t = obj.interval
        end
    end
end
    ...
    for name, obj in pairs(RemindrDB.reminders) do
        if type(obj) == 'table' and obj.t then
            ...
            if obj.t <= 0 then
                ...
                PlaySound(RemindrDB.AlarmSound)
                if obj.recurring then
                    obj.t = obj.interval
                else
                    RemindrDB.reminders[name] = nil
                end
            end
        end
    end
...
function SlashCmdList.REMINDR(msg, editbox)
    ...
    if cmd == "add" or cmd == "addr" then
        ...
        if args ~= "" then
            ...
            if timerName == "" or interval == "" or tonumber(interval) == nil or msg == "" then
                ...
                if RemindrDB.reminders[timerName] then
                    print(COLOR_ERROR .. "Reminder already exists with the name: " .. timerName)
                    return
                end
                local minutes = tonumber(interval)
                interval = interval * 60
                RemindrDB.reminders[timerName] = { interval=interval, msg=msg, recurring=isRecurring, t=interval }
    ...
    elseif cmd == "remove" then
        if args ~= "" then
            ...
            if RemindrDB.reminders[timerName] then
                RemindrDB.reminders[timerName] = nil
    ...
    elseif cmd == "list" then
        count = 0
        for _ in pairs(RemindrDB.reminders) do count = count + 1 end
        if count == 0 then
    ...
    elseif cmd == "config" then
        ...
        InterfaceOptionsFrame_OpenToCategory("Remindr")
        InterfaceOptionsFrame_OpenToCategory("Remindr")
